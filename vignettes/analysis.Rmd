---
title: "Source code from nitrogen uptake companion manuscripts"
...
This vignette contains R code used for data processing, statistical analysis, and production of figures for two associated manuscripts:

- "Nitrogen uptake and allocation by *Spartina alterniflora* and *Distichlis spicata*" (Hill et al., Journal of Experimental Marine Biology and Ecology)

- "Nitrogen and carbon concentrations and stable isotope ratios: data from a 15N tracer study in short-form *Spartina alterniflora* and *Distichlis spicata*" (Hill et al., Data In Brief)

Our R package contains four datasets, with associated documentation accessible using the standard '?' function: 
...
The datasets are also included as .csv files in the R package's inst/extdata subdirectory. 

For more information contact Troy Hill at Hill.Troy\@gmail.com.
...
```{r Allometry validation: comparison between predicted and harvested masses, include=FALSE}
###
...
```{r Figure 1 (Data In Brief), fig.width = 6, fig.height = 4}
...
```{r Relative growth rates, include=FALSE}
...
```{r stem density and primary production, include=FALSE}
...
```{r Denitrification enzyme activity and ambient N2O flux, include=FALSE}
...
```{r 15N inventories: aboveground, include=FALSE}
...
```{r N uptake rates, include=FALSE}
# N uptake rates ----------------------------------------------------------
...
```{r Results section A, include=FALSE}
...
``` {r Allometry model table, message = FALSE, echo = FALSE}
CSP.coef

```

Allometry model parameters




``` {r Figure 1 - Allometry, fig.width = 6, fig.height = 4, message = FALSE}
...
```{r Results section B, include = FALSE}
# Results: b.	Leaf and stem biomass, N pools, and uptake ------------------
...
``` {r Figure 2 (Data In Brief), fig.width = 6, fig.height = 4, message = FALSE}
...
``` {r Figure 2 (JEMBE), fig.width = 6, fig.height = 4, message = FALSE}
...
``` {r Figure 3 (JEMBE), fig.width = 6, fig.height = 4, message = FALSE}
...
```{r Results: C: Belowground biomass and N pools, include = FALSE}
...
``` {r Figure 3 (Data In Brief), fig.width = 6, fig.height = 4}
...
```{r Recoveries and sorption, include = FALSE}
...
Anova(aov(n15_pgFR ~ timeDiff * factor(species), data = master), type = "III")
summary(lm(n15_pgFR ~ timeDiff * factor(species), data = master))
...
```{r Drivers of N uptake, include = FALSE}
...
``` {r Figures 4-5 (JEMBE), fig.width = 6, fig.height = 4}
...
```{r Estimating belowground production, include = FALSE}
### differences in total nitrogen interception between species
...
```{r Results section E, echo = FALSE}
