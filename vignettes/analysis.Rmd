...
```{r load data}
# files <- list.files(system.file('extdata', package = 'NitrogenUptake2016'), full.names = TRUE)
# allom.sub <- read.csv(files[1])
# dat1      <- read.csv(files[2])
# dea       <- read.csv(files[3])
# test2     <- read.csv(files[4])
...
CSP <- plyr::dlply(allom.sub, c("spp"), bCM)
CSP.coef <- plyr::ldply(CSP, stats::coef)
CSP.coef$lam <- plyr::ddply(allom.sub, c("spp"), function(df)  bCM(df, lam.only = TRUE))[, "V1"]
...
dat.ld <- plyr::ddply(dat1, .(core_num, species, date), plyr::summarise,
...
### observed values
obs <- plyr::ddply(test2[test2$sample.type %in% c("leaf 1", "leaf 2", "leaf 3", "leaf 4", # "dead leaf",
...
mean.rgr     <- plyr::ddply(dat1[dat1$dead_live %in% "L", ], .(id, new.core.id, species), plyr::summarise, meanRGR = mean(RGR, na.rm = TRUE))
...
mean.dens     <- plyr::ddply(dat1[dat1$dead_live %in% "L", ], .(new.core.id, date), plyr::summarise, stem.density = sum(!is.na(height_cm)))
...
bkd <- plyr::ddply(test2[as.character(test2$time) %in% "t0", ], .(species, pool_label), plyr::summarise,
...
summary(stats::aov(n15xs ~ species, data = test2[(test2$sample.type %in% "dead biomass") & (!test2$time %in% "t0"), ]))
summary(stats::aov(n15xs ~ species, data = test2[(test2$sample.type %in% "dead biomass") & (!test2$time %in% "t0") & (test2$depth_bottom < 11), ])) # doesn't change if focused only on top 10 cm

### does sorption change over time? No
summary(stats::lm(n15xs ~ species + as.numeric(substr(time, 2, 2)), data = test2[(test2$sample.type %in% "dead biomass") & (!test2$time %in% "t0"), ])) # no
summary(stats::lm(n15xs ~ as.numeric(substr(time, 2, 2)), data = test2[(test2$sample.type %in% "dead biomass") & (!test2$time %in% "t0"), ]))
summary(stats::lm(n15xs ~ as.numeric(substr(time, 2, 2)), data = test2[(test2$sample.type %in% "dead biomass") & (!test2$time %in% "t0") & (test2$depth_bottom < 11), ])) # doesn't change if focused only on top 10 cm
...
mat2 <- plyr::ddply(test2[!test2$sample.type %in% c("bulk sediment"), ], .(time, species, new.core.id), plyr::summarise,
...
bgd <- plyr::ddply(test2[test2$sample.type %in% c("coarse roots", "fine roots", "rhizomes"), ],
...
abv <- plyr::ddply(test2[test2$sample.type %in% c("decomp layer", "leaf 1", "leaf 2", "leaf 3", "leaf 4",
                                            "leaf 5", "leaf 6", "leaf 7", "leaf 8", "leaf 9", "leaf 10", "leaf 11",
...
ag2 <- plyr::ddply(test2[test2$sample.type2 %in% c("stems", "leaf"), ],
...
bg <- plyr::ddply(test2[test2$sample.type %in% c("coarse roots", "fine roots", "rhizomes"), ],
...
# aboveground
ag <- plyr::ddply(test2[test2$sample.type %in% c("belowground stems", "stems", "leaf", paste0("leaf ", 1:10)), ],
            .(time, species, new.core.id), plyr::summarise,
            g   = sum(g_core / pot.m2, na.rm  = T),
            n_pct = mean(n_pct, na.rm = TRUE),
            n   = sum(n_core / pot.m2, na.rm  = T),
...
# species-level belowground inventory over time
bgd <- plyr::ddply(test2[test2$sample.type %in% c("coarse roots", "fine roots", "rhizomes"), ], #, "dead biomass"
             .(time, species, new.core.id), plyr::summarise,
             n15    = sum(n15_g, na.rm  = T)
)
...
tot.live <- plyr::ddply(test2[test2$sample.type %in% c("coarse roots", "fine roots", "rhizomes",
...
n_mean <- plyr::ddply(test2[test2$sample.type %in% c("belowground stems", "stems", "leaf", paste0("leaf ", 1:10)), ],
...
n_bg_mean <- plyr::ddply(test2[test2$sample.type %in% c("fine roots", "coarse roots", "rhizomes"), ],
                   .(species, time, new.core.id), plyr::summarise,
...
leaf <- plyr::ddply(test2[test2$sample.type %in% c("leaf", paste0("leaf ", 1:10)), ],
              .(species, time, new.core.id), plyr::summarise,
              tot_mass = sum(g_core / pot.m2, na.rm = TRUE),
              tot_n    = sum(n_core / pot.m2, na.rm = TRUE),
              tot_c    = sum(c_pct * g_core / pot.m2, na.rm = TRUE),
...
stem <- plyr::ddply(test2[test2$sample.type %in% c("belowground stems", "stems"), ],
              .(species, time, new.core.id), plyr::summarise,
              tot_mass = sum(g_core / pot.m2, na.rm = TRUE),
              tot_n    = sum(n_core / pot.m2, na.rm = TRUE),
              tot_c    = sum(c_pct * g_core / pot.m2, na.rm = TRUE),
...
rts <- plyr::ddply(test2[test2$sample.type %in% c("fine roots", "coarse roots", "rhizomes"), ],
...
graphics::plot(sample ~ height_cm, data = allom.sub[(allom.sub$spp %in% "DISP"), ],
...
graphics::points(sample ~ height_cm, data = allom.sub[(allom.sub$spp %in% "SPAL"), ],  cex = pointSize / 2, pch = 17)
