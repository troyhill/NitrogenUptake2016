...
plyr::ddply(master, .(species), summarise,
...
#ggsave(paste0("C:/RDATA/greenhouse/output/GRO/Figure3_", todaysDate,".png"), width = 80, height= 160, units = "mm", dpi = 400, gridExtra::arrangeGrob(fig3A, fig3B))
...
```{r Results: C: Belowground biomass and N pools }
...
summary(lm.mv.sa <- stats::lm(tot_15n_uptake ~ napp.MH + rgr + g_roots + session, data = master[master$species %in% "SA", ]))
summary(lm.mv.ds <- stats::lm(tot_15n_uptake ~ napp.MH + rgr + g_roots + session, data = master[master$species %in% "DS", ]))
