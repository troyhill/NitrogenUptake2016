...
library(MuMIn) # has dredge() function
library(zoo)
...
core.area <- pot.m2 <- 0.00801185 # pi * (0.1/2)^2 # pot area in m2; e.g., 5 g biomass per pot = 5*(1/0.008012) g/m2
...
CSP.coef$lam <- ddply(allom.sub, c("spp"), function(df)  lam(df))[, "V1"]
ldply(CSP, BIC)
...
for(i in 1:length(unique(dat1$id[dat1$dead_live %in% "L"]))) { # 830 plants
  ...
  if (nrow(subDat) > 1) {
    ...
    # plot(height ~ day, data = subDat)
    # plot(RGR ~ height, data = subDat)
...
# Figure - aboveground pools ---------------------------------------
ggplot(ag2.sp, aes(y = g, x = session, colour = species2)) + geom_point(position = dodge, size = pointSize) + theme_classic() %+replace% theme(legend.title = element_blank()) +
  facet_grid(sample.type2 ~ .) + geom_errorbar(aes(ymin = g - g.se, ymax = g + g.se), position = dodge, width = 0) +
...
ggplot(ag2.sp, aes(y = n_pct, x = session, colour = species2)) + geom_point(position = dodge, size = pointSize) + theme_classic() %+replace% theme(legend.title = element_blank()) +
  facet_grid(sample.type2 ~ .) + geom_errorbar(aes(ymin = n_pct - n_pct.se, ymax = n_pct + n_pct.se), position = dodge, width = 0) +
  scale_colour_grey(start = 0.5, end = 0.1, name = "", breaks = c(unique(ag2.sp$species2)[1], unique(ag2.sp$species)[2]), labels = c(expression(italic(Distichlis)), expression(italic(Spartina)))) +
...
# Figure - belowground biomass pools --------------------------------------
ggplot(bg.sp, aes(y = g, x = session, colour = species2)) + geom_point(position = dodge, size = pointSize) + theme_classic() +
  facet_grid(sample.type ~ .) + geom_errorbar(aes(ymin = g - g.se, ymax = g + g.se), position = dodge, width = 0) +
  ...
  #scale_shape_manual(values = c(16, 17), name = "", breaks = c(unique(bg.sp$species2)[1], unique(bg.sp$species)[2]), labels = c(expression(italic(D.~spicata)), expression(italic(S.~alterniflora)))) +
...
ggplot(bg.sp, aes(y = n_pct, x = session, colour = species2)) + geom_point(position = dodge, size = pointSize) + theme_classic() +
  facet_grid(sample.type ~ .) + geom_errorbar(aes(ymin = n_pct - n_pct.se, ymax = n_pct + n_pct.se), position = dodge, width = 0) +
  scale_colour_grey(start = 0.5, end = 0.1, name = "", breaks = c(unique(bg.sp$species2)[1], unique(bg.sp$species)[2]), labels = c(expression(italic(D.~spicata)), expression(italic(S.~alterniflora)))) +
  #scale_shape_manual(values = c(16, 17), name = "", breaks = c(unique(bg.sp$species2)[1], unique(bg.sp$species)[2]), labels = c(expression(italic(D.~spicata)), expression(italic(S.~alterniflora)))) +
